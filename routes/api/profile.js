...
router.get('/all', (req,res) => {
    ...
    .then( profiles => {
        if(!profiles){
            errors.noprofiles = 'There is no profiles yet';
            return res.status(404).json(errors);
        } else {
            res.json(profiles);
        }
...
router.get('/handle/:handle', (req,res) => {
    ...
    .then( profile => {
        if(!profile) {
            errors.noprofile = 'this handle has no profile';
...
router.get('/user/:user_id', (req,res) => {
    ...
    .then( profile => {
        if(!profile) {
            errors.noprofile = 'this handlea has no profile';
...
router.post('/experience', passport.authenticate('jwt', {session:false}), (req,res) => {
    ...
    .then( profile => {
        ...
        // Add newExp to experience array
